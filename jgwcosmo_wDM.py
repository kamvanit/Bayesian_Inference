...
def Ez_inv(z,Om0,w):
    ...
    #return 1./jnp.sqrt( (1.-Om0)*jnp.power( 1.+z, 3*(1+w)) + Om0*jnp.power( 1.+z, 3 ) )     # constraining wDE
    return 1./jnp.sqrt( (1.-Om0) + Om0*jnp.power( 1.+z, 3*(1+w) ) )     # constraining wDM
...
def diff_comoving_volume(z,H0,Om0,w):
    dL = d_L(z,H0,Om0,w) #Mpc
    Ez_i = Ez_inv(z,Om0,w)
    D_H = (Clight/1e3)  / H0 #Mpc
    return  1.0e-9 * (4.*jnp.pi) * jnp.power(dL,2) * D_H * Ez_i / jnp.power(1.+z,2.)
 
    
def z_at_dl(dl,H0,Om0,w,zmin=1e-3,zmax=100):
    #dl in Mpc
    zs = jnp.logspace(jnp.log10(zmin),jnp.log10(zmax),1000)
    return jnp.interp(dl, d_L(zs,H0,Om0,w),zs, left=zmin, right=zmax, period=None)

def detector_to_source_frame(m1z,m2z,dL,H0,Om0,w,zmin=1e-3,zmax=100):
    z = z_at_dl(dL,H0,Om0,w,zmin,zmax)
    m1 = m1z / (1. + z)
    m2 = m2z / (1. + z)
    return m1, m2, z
