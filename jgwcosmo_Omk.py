...
def Ez_inv(z,Om0,Omk):
    #return 1./jnp.sqrt((1.-Om0) + Om0*jnp.power((1.+z),3) ) #flat universe version, and constant DE

    #return 1./jnp.sqrt( (1.-Om0)*jnp.power( 1.+z, -3*(1+w)) + Om0*jnp.power( 1.+z, 3 ) )     # constraining w
    Ode = 1- Omk - Om0   
    return 1./jnp.sqrt( Ode + Omk*jnp.power(1.+z,2) + Om0*jnp.power((1.+z),3) )


@jnp.vectorize
def d_L(z,H0,Om0,Omk):
    zs = jnp.linspace(0,z,100)
    #integral = jnp.trapz(Ez_inv(zs,Om0,w),zs)
    #return (1+z)*integral * (Clight/1e3)  / H0 #Mpc

    factor = (1.+z)*(Clight/1e3) /jnp.sqrt( jnp.abs(Omk)) /H0
    integral = jnp.sqrt( jnp.abs(Omk)) * jnp.trapz(Ez_inv(zs,Om0,Omk),zs)
    
    #minus_k = factor * jnp.arcsinh( integral )
    #plus_k = jnp.where(Omk>0, factor * jnp.arcsin( integral ), minus_k)
    return factor * jnp.arcsinh( integral )
    
def diff_comoving_volume(z,H0,Om0,Omk):
    dL = d_L(z,H0,Om0,Omk) #Mpc
    Ez_i = Ez_inv(z,Om0,Omk)
    D_H = (Clight/1e3)  / H0 #Mpc
    return  1.0e-9 * (4.*jnp.pi) * jnp.power(dL,2) * D_H * Ez_i / jnp.power(1.+z,2.)
 
    
def z_at_dl(dl,H0,Om0,Omk,zmin=1e-3,zmax=100):
    #dl in Mpc
    zs = jnp.logspace(jnp.log10(zmin),jnp.log10(zmax),1000)
    return jnp.interp(dl, d_L(zs,H0,Om0,Omk),zs, left=zmin, right=zmax, period=None)

def detector_to_source_frame(m1z,m2z,dL,H0,Om0,Omk,zmin=1e-3,zmax=100):
    z = z_at_dl(dL,H0,Om0,Omk,zmin,zmax)
    m1 = m1z / (1. + z)
    m2 = m2z / (1. + z)
    return m1, m2, z
...
def dL_approx(z,H0,Om0):
    ...
    #zs = jnp.linspace(0,z,10000)                 #******************************************
    #integral = jnp.trapz(Ez_inv(zs,Om0),zs)    
    #print(integral.shape)
    #return (1.+z)*(jnp.sqrt(jnp.abs(Om0))*integral  )/H0/jnp.sqrt(jnp.abs(Om0))
